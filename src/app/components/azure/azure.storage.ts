...
export async function uploadFile2(content: MyFileData, newContainer?: string, file?: Blob, isFinished = false) {
  ...
  if (content.file.type){
    if (content.file.type.match(/^image/) || content.file.type === 'application/pdf') {
      await blockBlobClient.setHTTPHeaders({ blobContentDisposition: `inline;filename="${content.fileName}"` })
    }
  }else{
    if ((content.file as any).properties?.contentDisposition){
      await blockBlobClient.setHTTPHeaders({ blobContentDisposition: (content.file as any).properties?.contentDisposition })
