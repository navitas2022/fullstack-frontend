  ...
  attachFile(ev: any) {
    ...
    input.onchange = async (e: any) => {
      ...
      if (!this.dataTable.length) {
        this.dataTable.push([file, (file.size / 1024).toFixed(0) + "kb", String(new Date()).substring(0, 24), "Description", "contents"])
      }
      else {
        let conf = confirm("Only one file allowed. Do you want to replace the file?")
        if (conf) {
          await deleteBlob("coverpage", this.dataTable[0][0].name)
          this.dataTable[0] = [file, (file.size / 1024).toFixed(0) + "kb", String(new Date()).substring(0, 24), "Description", "contents"]
        }
      }
    }
  }
